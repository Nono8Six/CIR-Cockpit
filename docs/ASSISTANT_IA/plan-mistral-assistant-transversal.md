...
3. Chantier 3 — tolérance lexicale pluriels/typos (P5) : implémenté, poussé et déployé en v198 le 2026-07-23 ; couche de recherche locale verte, mais checkpoint end-to-end maintenu ouvert après divergence singulier/pluriel et autocorrection de `debimetre` par la passe 1.
...
- Preuves locales CP-C3 : singulier/pluriel servomoteurs → même forme `servomoteur electrique`, 1 CAT_FAB déterministe et 1 identité candidate sémantique pour chaque formulation sur le snapshot actif ; `debimetre` → unique suggestion `Capteurs/débitmètres` à 0,615385 ; `EXPLAIN ANALYZE` 147 ms sur 6 816 libellés, 0 bloc disque lu. Tests ciblés CP-C3 36/36, suite IA élargie 93/93, `deno check` et `qa:back` verts (449 réussites / 0 échec / 14 intégrations ignorées)
- Livraison CP-C2/CP-C3 du 2026-07-23 : commit `1f2c61c` poussé sur `origin/codex/mistral-phase-1b-total`, Edge Function `api` v198 ACTIVE, wrapper `supabase/functions/api/index.ts`, import map racine et `verify_jwt=false` confirmés. `OPTIONS 200`, appel sans Bearer `401 AUTH_REQUIRED`. Aucune migration appliquée : aucun DDL CP-C3 et parité locale/distante déjà complète
- Réserve runtime CP-C3 : sur v198, la passe 1 Mistral produit des plans différents pour les formulations singulier/pluriel, puis la qualification retourne 116 couples / 5 marques (`1ae1f956…`) contre 87 / 6 (`593c015e…`). `debimetre` est autocorrigé avant le zéro-candidat et retourne 25 / 6 (`088bab5e…`, deux rounds, 0,011528 USD) au lieu de la suggestion locale. La tolérance de la couche SQL est prouvée, mais le contrat end-to-end CP-C3 reste ouvert
- Rollback model-first après v198 : `AI_ASSISTANT_MODEL_ROUTING_ENABLED=false` confirmé par la question CP-C2, qui retourne la clarification FAM/CAT_FAB sans outil, usage ni provider (`9dd3ed65…`)
...
| P4 | Version minimale implémentée, prouvée et livrée dans le commit `1f2c61c`, non activée | Routage model-first prouvé sur servomoteurs ; fast-paths zéro token et rollback prouvés ; `AI_ASSISTANT_MODEL_ROUTING_ENABLED=false` confirmé sur l'API v198. Recommandation : GO du code CP-C2, NO-GO d'activation jusqu'à stabilisation end-to-end. |
| P5 | GO contrôlé le 2026-07-20, NO-GO global maintenu ; CP-C3 déployé mais checkpoint ouvert | Vérins (75 couples, 5 marques), variateur (308 couples, 6 marques, ancrage révisé validé au CP-C1) et débitmètres (25 couples, 6 marques) prouvés ; folding et suggestion trigram verts en tests + SQL direct, mais v198 diverge end-to-end sur singulier/pluriel et la passe 1 autocorrige `debimetre` avant la suggestion ; `AI_ASSISTANT_SEMANTIC_PLANNER_ENABLED=true`, routage model-first maintenu à `false` |
