...
> Lire `00-plan-general.md`. Périmètre : frontend. Gate QA : `pnpm run qa:front`.
...
1. **Vue d'ensemble** — état du provider (actif, clé présente/last4, dernier test), modèle par
   défaut, coût 30 j, appels/tokens, alertes budget (si phase 6 posée, sinon placeholder),
...
2. **Fournisseur & modèles** — gestion **multi-modèles** réelle : liste des modèles
   ...
   `assistant.referentiels` vs `pricing.references.diagnose`). Clé API OpenRouter
   (saisie/remplacement/test), jamais affichée en clair.
...
router + miroir + test), en respectant `cir-cockpit-api-contracts`. Ne pas bricoler côté front
ce qui doit être une vraie procédure.
...
- [ ] Onglet Fournisseur & modèles : CRUD multi-modèles + défaut par feature + clé (test, jamais en clair).
...
- [ ] Plus aucun uuid brut affiché ; plus aucune valeur provider/modèle hardcodée en dur bloquante.
- [ ] Services front complétés (pattern invokeTrpc/parseResponse) ; procédures backend manquantes ajoutées proprement si besoin.
- [ ] Tests Vitest à jour (dont résolution des noms) ; `pnpm run qa:front` vert.
...
supprimer les valeurs hardcodées bloquantes, brancher l'onglet Prompts sur l'API existante,
...
Lance `pnpm run qa:front` jusqu'au vert.
