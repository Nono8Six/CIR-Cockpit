...
## BE-P1-03 — Remplacement Edge → Node achevé dans le runtime réel
...
- **Conditions initialement envisagées :** accès aux journaux avec métadonnées suffisantes ; inventaire des builds/frontends/automatisations encore actifs ; validation du PO sur la fenêtre d'observation et la fin du configurateur moteur.
- **Critères d'acceptation initialement envisagés :** matrice exhaustive des actions observées et de leurs consommateurs ; migration prouvée de chaque consommateur vers la procédure tRPC cible ; absence de requête métier vers `functions/v1/api` pendant la fenêtre définie ; puis désactivation et sonde post-cutover. Le PO a finalement autorisé le retrait immédiat en acceptant le risque résiduel documenté. Depuis ce retrait, un `404` sur l'ancien endpoint est le comportement attendu, pas un échec du cutover.

## Décision de séquencement

La Brique DBOS est **conditionnée**, pas abandonnée. La condition de topologie Edge → Node est acquise. Elle ne doit commencer qu'après :

1. la frontière DB/RLS de BE-P1-01 et l'acteur d'audit de BE-P1-05 ;
2. la stabilisation des réservations IA de BE-P2-02.

Ce séquencement évite de rendre durable une frontière de données dont l'identité RLS et l'acteur d'audit ne sont pas encore établis.
