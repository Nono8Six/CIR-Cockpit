...
const PromptTotalsSummary = ({ prompts }: { prompts: AiPromptWithVersions[] }) => {
  const summary = useMemo(() => {
    return prompts.reduce(
      (current, prompt) => ({
        total: current.total + 1,
        available: current.available + (prompt.archived_at ? 0 : 1),
        archived: current.archived + (prompt.archived_at ? 1 : 0),
        calls: current.calls + prompt.usage.calls,
        callsLast30Days: current.callsLast30Days + prompt.usage.calls_last_30_days,
        tokens: current.tokens + prompt.usage.total_tokens,
        cost: current.cost + prompt.usage.cost_amount,
      }),
      { total: 0, available: 0, archived: 0, calls: 0, callsLast30Days: 0, tokens: 0, cost: 0 },
    );
  }, [prompts]);

  return (
    <div
      className="flex flex-wrap items-baseline gap-2 border-y border-border py-2.5 text-[11px] text-muted-foreground"
      data-testid="prompt-studio-totals"
    >
      <span className="font-mono font-medium tabular-nums text-foreground">
        {formatNumber.format(summary.total)} {summary.total > 1 ? 'templates' : 'template'}
      </span>
      <span aria-hidden="true">·</span>
      <span className="font-mono tabular-nums">
        {formatNumber.format(summary.available)} {summary.available > 1 ? 'disponibles' : 'disponible'}
      </span>
      <span aria-hidden="true">·</span>
      <span className="font-mono tabular-nums">
        {formatNumber.format(summary.archived)} {summary.archived > 1 ? 'archivés' : 'archivé'}
      </span>
      <span aria-hidden="true">·</span>
      <span className="font-mono tabular-nums">
        {formatNumber.format(summary.calls)} appels ({formatNumber.format(summary.callsLast30Days)} sur 30j)
      </span>
      <span aria-hidden="true">·</span>
      <span className="font-mono tabular-nums">
        {formatNumber.format(summary.tokens)} tokens
      </span>
      <span aria-hidden="true">·</span>
      <span className="font-mono tabular-nums font-medium text-foreground">
        {formatCost.format(summary.cost)} total
      </span>
    </div>
  );
};

export const AiPromptStudioView = () => {
  ...
  return (
    <div className="space-y-5" data-testid="ai-prompt-studio-view">
      ...
      <PromptTotalsSummary prompts={prompts} />
          ...
          <Table className="min-w-[980px]">
            ...
            <TableBody>
              {filteredPrompts.map((prompt) => {
                ...
                const canDelete = isArchived && prompt.usage.calls === 0;
                return (
