import { ShieldAlert, Monitor, CheckSquare, Layers } from 'lucide-react';
...
/**
 * Bento card representing Product-wide global settings and defaults.
 * These are visible/editable by super-admins.
 *
 * @param {ProductSettingsCardProps} props - The component props.
 * @param {boolean} props.productAllowManualEntry - Whether manual entry is allowed by default.
 * @param {'term' | 'cash'} props.productDefaultCompanyAccountType - The default account type for companies.
 * @param {boolean} props.productUiShellV2 - Whether UI shell v2 is enabled.
 * @param {boolean} props.readOnly - Whether the fields are read-only.
 * @param {function} props.setProductAllowManualEntry - Callback to update productAllowManualEntry.
 * @param {function} props.setProductDefaultCompanyAccountType - Callback to update productDefaultCompanyAccountType.
 * @param {function} props.setProductUiShellV2 - Callback to update productUiShellV2.
 * @returns {JSX.Element} The rendered product settings card.
 */
  ...
  return (
    ...
    >
      ...
      <div className="space-y-5">
        ...
        <div className="flex flex-col gap-3 rounded-lg border border-border bg-surface-1/40 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-0.5">
            ...
            <p className="text-[11px] text-muted-foreground leading-relaxed">
              Activer la nouvelle barre de navigation latérale et le shell de l&apos;application.
            </p>
          </div>
        ...
        {/* Dropdown for default company account type */}
        <div className="flex flex-col gap-2 rounded-lg border border-border bg-surface-1/40 p-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex shrink-0">
            <select
              ...
              disabled={readOnly}
              onChange={(e) => setProductDefaultCompanyAccountType(e.target.value as 'term' | 'cash')}
              className="w-full rounded-md border border-border bg-background px-2.5 py-1.5 text-xs text-foreground shadow-sm transition-all focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary/20 disabled:cursor-not-allowed disabled:opacity-50"
            >
