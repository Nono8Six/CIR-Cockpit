        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-border bg-card p-5 shadow-sm space-y-4">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0 space-y-1">
                <span className="text-[11px] font-semibold uppercase tracking-wide text-primary">Aperçu fiche</span>
              ...
              {draft.official_data_source ? (
                <div className="flex items-center gap-1.5">
                  ...
                  <Badge variant="success" className="text-[11px] px-1.5 py-0 border-success/30 bg-success/5 text-success">OFFICIEL</Badge>
                </div>
              ) : (
                <div className="flex items-center gap-1.5">
                  <span className="h-1.5 w-1.5 rounded-full bg-warning shadow-[0_0_0_2px_hsl(var(--warning)/0.14)]" aria-hidden="true" />
                  <Badge variant="outline" className="text-[11px] px-1.5 py-0 border-primary/30 bg-primary/5 text-primary">MANUEL</Badge>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-2.5 pt-1">
              <div className="rounded-lg border border-border-subtle bg-surface-2/40 p-2.5 space-y-0.5">
                <p className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Code</p>
                <p className="font-mono text-xs font-bold text-foreground truncate">{draft.supplier_code || '—'}</p>
              </div>
              <div className="rounded-lg border border-border-subtle bg-surface-2/40 p-2.5 space-y-0.5">
                <p className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">N° Interne</p>
            ...
            {draft.siret || draft.naf_code || draft.city ? (
              <div className="flex flex-wrap gap-1.5 pt-2 border-t border-border-subtle">
                {draft.siret ? <Badge variant="outline" className="text-[11px] font-mono bg-background/50">SIRET {draft.siret}</Badge> : null}
                {draft.naf_code ? <Badge variant="outline" className="text-[11px] font-mono bg-background/50">NAF {draft.naf_code}</Badge> : null}
              </div>
            ) : null}
          </div>
          ...
          {step === 'details' && (
            <div className="rounded-lg border border-primary/10 bg-primary/[0.01] p-4 text-xs text-muted-foreground leading-relaxed space-y-2">
              ...
              <p>Complétez les champs obligatoires marqués d&apos;un astérisque rouge pour débloquer l&apos;étape de validation finale.</p>
            </div>
          )}
        </div>
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-border bg-card p-5 shadow-sm space-y-4">
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="flex items-center gap-1.5">
                <span className="h-1.5 w-1.5 rounded-full bg-success shadow-[0_0_0_2px_hsl(var(--success)/0.14)]" aria-hidden="true" />
                <Badge variant="success" className="text-[11px] px-1.5 py-0">Officiel</Badge>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-lg border border-border-subtle bg-surface-2/40 p-2.5">
                <p className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">SIREN</p>
          ...
          {selectedCompany ? (
            <div className="rounded-xl border border-border bg-card p-5 shadow-sm space-y-4">
              <div className="flex items-start justify-between gap-3 border-b border-border-subtle pb-3">
                <div>
                  <p className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Établissement sélectionné</p>
              ...
              <dl className="space-y-3 text-xs font-medium text-foreground pt-1">
                {selectedCompany.siret ? (
                  <div className="flex flex-col gap-0.5">
                    <dt className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">SIRET</dt>
                    <dd className="font-mono font-semibold">{selectedCompany.siret}</dd>
                  </div>
                ) : null}
                {selectedCompany.address ? (
                  <div className="flex flex-col gap-0.5">
                    <dt className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Adresse</dt>
                    <dd className="font-semibold">{selectedCompany.address}</dd>
                  </div>
                ) : null}
                {selectedCompany.city ? (
                  <div className="flex flex-col gap-0.5">
                    <dt className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Ville</dt>
                    <dd className="font-semibold">{[selectedCompany.postal_code, selectedCompany.city].filter(Boolean).join(' ')}</dd>
                  </div>
                ) : null}
                {selectedCompany.region ? (
                  <div className="flex flex-col gap-0.5">
                    <dt className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Région</dt>
                ...
                {formatOfficialNaf(selectedCompany.naf_code) ? (
                  <div className="flex flex-col gap-0.5">
                    <dt className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Activité</dt>
                    <dd className="font-semibold">{formatOfficialNaf(selectedCompany.naf_code)}</dd>
                  </div>
                ) : null}
                {selectedCompany.date_debut_activite ? (
                  <div className="flex flex-col gap-0.5">
                    <dt className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Début d&apos;activité</dt>
                ...
                {selectedCompany.establishment_closed_at ? (
                  <div className="flex flex-col gap-0.5">
                    <dt className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Fermeture</dt>
            ...
            <div className="rounded-xl border border-dashed border-border bg-card p-6 text-center text-xs font-semibold text-muted-foreground leading-normal">
              Sélectionnez un établissement dans la liste pour en prévisualiser les données complètes.
            </div>
          )}
        </div>
      ) : (
