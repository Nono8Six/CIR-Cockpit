...
describe("useAssistantChat", () => {
  ...
  it("borne à 12 messages l’historique envoyé au service", async () => {
    ...
    expect(seventhCall?.history[0]).toEqual({
      role: "user",
      content: "Question 1",
    });
    expect(seventhCall?.history[11]).toEqual({
      role: "assistant",
  ...
  it("réutilise le client_request_id au retry puis en génère un autre au nouvel envoi", async () => {
    ...
    const firstRequestId = vi.mocked(askAiAssistant).mock.calls[0]?.[0]
      .client_request_id;

    await act(async () => {
      await result.current.retry();
    });
    expect(vi.mocked(askAiAssistant).mock.calls[1]?.[0].client_request_id).toBe(
      firstRequestId,
    );
    ...
    expect(vi.mocked(askAiAssistant).mock.calls[2]?.[0].client_request_id).not
      .toBe(firstRequestId);
  });
  ...
  it("génère une nouvelle requête pour relancer après une erreur fournisseur confirmée", async () => {
    ...
    const failedRequestId = vi.mocked(askAiAssistant).mock.calls[0]?.[0]
      .client_request_id;
    ...
    expect(vi.mocked(askAiAssistant).mock.calls[1]?.[0].client_request_id).not
      .toBe(failedRequestId);
  });

  it("transporte uniquement le dernier contexte structuré et le réinitialise", async () => {
    vi.mocked(askAiAssistant)
      .mockResolvedValueOnce(
        successfulResponse("Recherche trouvée", conversationContext),
      )
      .mockResolvedValueOnce(
        successfulResponse("ROCK correspond", conversationContext),
      )
    ...
    expect(vi.mocked(askAiAssistant).mock.calls[0]?.[0].conversation_context)
      .toBeNull();
