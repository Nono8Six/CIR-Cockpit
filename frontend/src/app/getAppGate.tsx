...
const ProfileErrorScreen = ({ profileError, onProfileRetry, onSignOut }: {
  ...
  <div className="min-h-screen w-full flex items-center justify-center bg-slate-50/70 text-slate-500 font-sans p-6">
    <div className="w-full max-w-md bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
      <h1 className="text-lg font-semibold text-slate-900">Profil indisponible</h1>
      <p className="text-sm text-slate-600 mt-2">{profileError}</p>
...
const ProfileMissingScreen = ({ onSignOut }: { onSignOut: () => void }) => (
  <div className="min-h-screen w-full flex items-center justify-center bg-slate-50/70 text-slate-500 font-sans p-6">
    <div className="w-full max-w-md bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
      <h1 className="text-lg font-semibold text-slate-900">Profil introuvable</h1>
      <p className="text-sm text-slate-600 mt-2">Votre profil n&apos;a pas pu etre charge. Veuillez vous reconnecter.</p>
...
export const getAppGate = ({
  ...
  if (mustChangePassword) {
    return <ChangePasswordScreen userEmail={profile.email} onComplete={onPasswordChanged} onSignOut={onSignOut} />;
  }
  return null;
};
