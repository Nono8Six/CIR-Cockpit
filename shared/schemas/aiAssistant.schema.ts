...
const aiAssistantProductSemanticClarificationContextSchema = z.strictObject({
  ...aiAssistantConversationContextBaseShape,
  kind: z.literal("product_semantic_clarification"),
  intent: z.literal("product_semantic_search"),
  ...
  options: z.array(
    z.string().trim().min(1, { error: "Option de clarification requise." })
      .max(160, { error: "Option de clarification trop longue." }),
  ).min(2, { error: "Deux options de clarification sont requises." }).max(5, {
    error: "Maximum 5 options de clarification.",
  }),
  snapshot_id: uuidSchema,
});

const aiAssistantProductSemanticSelectionSchema = z.discriminatedUnion(
  "kind",
  [
    z.strictObject({
      kind: z.literal("classification_scope"),
      cir_path: z.string().trim().min(1, {
        error: "Chemin CIR qualifié requis.",
      }).max(600, { error: "Chemin CIR qualifié trop long." }),
    }),
    z.strictObject({
      kind: z.literal("direct_label"),
      normalized_cat_fab: z.string().trim().min(1, {
        error: "CAT_FAB qualifiée requise.",
      }).max(240, { error: "CAT_FAB qualifiée trop longue." }),
      cir_path: z.string().max(600, {
        error: "Chemin CIR qualifié trop long.",
      }),
    }),
  ],
);

const aiAssistantProductSemanticResultContextSchema = z.strictObject({
  ...aiAssistantConversationContextBaseShape,
  kind: z.literal("product_semantic_result"),
  intent: z.literal("product_semantic_search"),
  concept: z.string().trim().min(1, { error: "Concept produit requis." }).max(
    160,
    { error: "Concept produit trop long." },
  ...
  snapshot_id: uuidSchema,
  source_client_request_id: uuidSchema,
  accepted_selections: z.array(aiAssistantProductSemanticSelectionSchema).max(
    80,
    { error: "Maximum 80 sélections produit qualifiées." },
  ),
  result_summary: z.strictObject({
    matching_brands: assistantBoundedBrandsSchema,
    distinct_brand_count: z.number().int().nonnegative(),
    distinct_brand_cat_fab: z.number().int().nonnegative(),
  }),
});
...
export const aiAssistantConversationContextSchema = z.discriminatedUnion(
  ...
  [
    ...
    aiAssistantProductSemanticClarificationContextSchema,
    aiAssistantProductSemanticResultContextSchema,
  ],
);
...
export const aiAssistantEvidenceSchema = z.strictObject({
  status: z.enum(["verified", "qualified", "partial", "failed"]),
  ...
  if (evidence.status === "qualified" && evidence.facts.length === 0) {
    context.addIssue({
      code: "custom",
      message: "Un resultat qualifie requiert au moins un fait recompte.",
      path: ["facts"],
    });
  }
