create or replace function private.sync_interaction_status()
returns trigger
language plpgsql
set search_path = ''
as $$
...
begin
  ...
  if NEW.entity_id is not null and NEW.agency_id is null then
    select e.agency_id into v_entity_agency_id
...
drop function if exists public.sync_interaction_status();
