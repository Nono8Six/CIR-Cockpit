-- La passe de planification voit la taxonomie CIR du snapshot et selectionne
-- des branches par sens ; les variantes lexicales restent permises, y compris
-- larges, pour les libelles CAT_FAB hors branche. La regle de portee par
-- libelle terminal et la regle anti-famille-parente sont conservees.
...
begin
  ...
  if not exists (
    ...
    where template_id = assistant_template_id
      and change_note = 'Passe taxonomie et selection de branches CIR'
  ) then
