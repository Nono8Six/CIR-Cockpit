...
import { type SqlCondition } from '../core/directoryShared.ts';
import {
...
export const getCompanyDuplicateMatches = async (
  ...
  return (rows as DirectoryDuplicateLookupRow[])
    .map((row: DirectoryDuplicateLookupRow) => {
    ...
    .filter((entry: DirectoryDuplicateMatch | null): entry is DirectoryDuplicateMatch => entry !== null);
};
