...
export const getDirectoryOptionCommercials = async (
  ...
  try {
    ...
    const commercials: DirectoryCommercialOption[] = Array.from(
      new Map(
        commercialRows
          .map(toCommercialOption)
          .filter((value: DirectoryCommercialOption | null): value is DirectoryCommercialOption => value !== null)
          .map((option: DirectoryCommercialOption) => [option.id, option] as [string, DirectoryCommercialOption]),
      ).values(),
    );
...
export const getDirectoryOptionDepartments = async (
  ...
  try {
    ...
    const departments: string[] = Array.from(
      new Set(
        departmentRows
          .map((row: { department: string | null }) => normalizeDepartment(row.department))
...
export const getDirectoryOptionCities = async (
  ...
  try {
    ...
    const cities: DirectorySuggestionOption[] = Array.from(
      new Set(
        cityRows
          .map((row: { city: string | null }) => normalizeNullableText(row.city))
          .filter((value: string | null): value is string => value !== null),
      ),
    ).map((city: string): DirectorySuggestionOption => ({
      value: city,
      label: city,
    }));
    const meta = toDirectoryResponseMeta(resolvedScope, input.debug?.includeResolvedScope);
