...
const REFERENCE_EXPECTATIONS = {
  ...
  rockDriveRows: 234,
  matchingBrands: ["BONF", "FEST", "LERO", "OPTI", "PARK", "REXR", "ROCK", "SIEM"],
  distinctBrands: 140,
} as const;
const P0_EXACT_CONVERSATIONS = [
  ["Combien il y a des familles produit chez FEST (FESTO) ? dans cat_fab"],
  ["Tu peux me dire les marques qui ont des familles de produits avec des variateurs ou drives, tu vois ?", "cat_fab"],
  ["tes sur ? et rock ?"],
...
Deno.test("P0 route la recherche CAT_FAB drive sans casse vers un outil exhaustif", () => {
  ...
  for (const term of candidates) {
    const question = `Tu peux me dire les marques qui ont des familles de produits avec des ${term} dans cat_fab ?`;
    assertEquals(
      selectAssistantTools(question, [{
        type: "function",
        function: {
...
Deno.test("P0 la relance et ROCK conserve la recherche precedente", () => {
  assertEquals(
    selectAssistantTools("tes sur ? et rock ?", [{
      type: "function",
      function: {
        name: "check_brand_matches",
        description: "Vérifie une marque dans la recherche précédente.",
        parameters: { type: "object", properties: {} },
      },
    }, ...tools]).map((tool) => tool.function.name),
    ["check_brand_matches"],
