...
Deno.test('P6 couvre les incidents et variations obligatoires avec 10 ou 20 repetitions', () => {
  assertEquals(P6_CASES.map((item) => item.id), [
    ...
    'p5b-top-remises-fest',
    'p5b-ecarts-remise-threshold',
    'p5b-search-schema-remises',
    'p5b-tri-text-financier-refuse',
  ]);
  assertEquals(
    P6_CASES.every((item) =>
...
Deno.test('P6 refuse provider, tokens et cout sur un chemin deterministe', () => {
  ...
  assertEquals(result.violations, ['provider_on_deterministic_path']);
});
...
Deno.test('P6 refuse un rapport contenant un secret ou un champ inconnu', () => {
  const report = {
    ...
    policy: {
      ...
      max_price_enforced: true,
      data_collection_filter: false,
      zdr_enforced: false,
    },
    executions: [],
    metrics: [],
  };
  assertEquals(assertEvaluationReportSafe(report).version, '6.1.0');
  assertThrows(() =>
