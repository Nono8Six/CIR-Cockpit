...
export const ASSISTANT_MODEL_EVALUATION_VERSION = "6.0.0" as const;
...
export const assistantEvaluationExecutionSchema = z.strictObject({
  ...
  provider_rounds: z.number().int().nonnegative().max(7),
  finish_reasons: z.array(z.string().trim().min(1).max(100)).max(7),
...
export const classifyEvaluation = (
  ...
  if (observation.providerRounds > expectation.maxProviderRounds) {
    violations.push("provider_round_limit");
  }
