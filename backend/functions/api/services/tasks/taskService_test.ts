...
Deno.test("a TCS creates only inside an assigned agency and cannot assign another user", () => {
...
Deno.test("an identical retry is accepted and an idempotency-key reuse is rejected", () => {
...
Deno.test("database timestamp values are normalized before strict contract parsing", () => {
  assertEquals(
    normalizeDatabaseDates({
...
Deno.test("B3-3 lifecycle only permits documented transitions and records reopening", () => {
...
Deno.test("B3-3 increments event order and rejects an obsolete task version", () => {
