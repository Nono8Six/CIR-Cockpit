...
const getProbeTierScope = async (): Promise<ProbeTierScope> => {
  ...
  try {
    const rows = await database.unsafe<ProbeTierScope[]>(
      `select e.agency_id as "agencyId", e.id as "organizationId", c.id as "contactId", a.id as "activityId"
       from public.entities e
       join public.entity_contacts c on c.entity_id = e.id and c.archived_at is null
       join public.activities a on a.organization_id = e.id and a.agency_id = e.agency_id
       where e.archived_at is null
       limit 1`,
    );
    const scope = rows[0];
    assert(scope, "Aucun Tier, contact et Activity actifs ne permet la probe.");
    return scope;
  } finally {
    await database.end({ timeout: 0 });
  }
...
const assertTaskRoutesCors = async () => {
  const url = Deno.env.get("SUPABASE_URL")?.trim();
  assert(url, "URL Supabase d intégration absente.");
  for (
    const route of [
      "task-types.list",
      "task-types.admin",
    ...
    const response = await fetch(`${url}/functions/v1/api/trpc/${route}`, {
      method: "OPTIONS",
      headers: {
        Origin: corsOrigin,
...
Deno.test({
  ...
  fn: async () => {
    ...
    const taskTypeId = readString(createdType.payload, "id");
    ...
    const tcsId = readString(createTcs.payload, "user_id");
    ...
    const agencyAdminId = readString(createAgencyAdmin.payload, "user_id");
    ...
    const observerId = readString(createObserver.payload, "user_id");
    assert(observerId, "Identifiant de l observateur de probe absent.");

    const tcsToken = await signInProbeUser(tcsEmail, password);
    const agencyAdminToken = await signInProbeUser(agencyAdminEmail, password);
    ...
    assertEquals(readString(readTask(quick.payload), "responsible_id"), tcsId);
    ...
    assertEquals(readString(invalidPayload.payload, "code"), "INVALID_PAYLOAD");
    ...
    const delegatedParticipants = readValue(delegated.payload, "participants");
    assert(
    ...
    const tierTask = await postApi("tasks.create", tcsToken, {
      ...
      source_activity_id: probeTierScope.activityId,
      responsible_id: tcsId,
      visibility: "tier",
      participants: [],
    });
    ...
    assertEquals(readString(readTask(claimQueue.payload), "responsible_id"), tcsId);
    ...
    const addContributor = await postApi("tasks.update-assignment", agencyAdminToken, {
      task_id: delegatedTaskId,
      expected_version: 4,
    ...
    const forbiddenContributorCompletion = await postApi("tasks.change-status", observerToken, {
      task_id: delegatedTaskId,
      expected_version: 6,
      status: "completed",
    });
    ...
    const history = await getApi("tasks.get", tcsToken, { task_id: delegatedTaskId });
    assertEquals(history.status, 200);
    const events = readValue(history.payload, "events");
    assert(Array.isArray(events) && events.length >= 8, "Historique B3-3 incomplet.");
    assertEquals(
    ...
    const archivedAssignment = await postApi("tasks.create", agencyAdminToken, {
      kind: "advanced",
      agency_id: agencyId,
