...
test('AI settings contracts are strict and never expose API keys', () => {
  const provider = {
...
test('AI mutation inputs keep secrets server-bound and strict', () => {
...
test('AI prompt and usage contracts are strict', () => {
...
test('watch summarize contract accepts a run selector and rejects client-supplied facts', () => {
  const runId = crypto.randomUUID();
  const targetSnapshotId = crypto.randomUUID();
  const clientRequestId = crypto.randomUUID();
  assertEquals(pricingReferencesWatchSummarizeInputSchema.safeParse({
    run_id: runId,
  ...
  assertEquals(pricingReferencesWatchSummarizeInputSchema.safeParse({
    run_id: runId,
    client_request_id: clientRequestId,
    facts: { summary: 'injected' }
  }).success, false);
  assertEquals(pricingReferencesWatchSummarizeResponseSchema.safeParse({
    ok: true,
    result: {
      summary: 'Aucun ecart.',
      summary_fact_ids: ['summary.total'],
      priority_anomalies: [],
      recommendations: [{ text: 'Relire le run.', fact_ids: ['summary.total'] }],
      limits: [],
      confidence: 0.4
    },
    usage: {
