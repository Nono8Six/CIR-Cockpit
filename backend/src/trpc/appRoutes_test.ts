...
import { assertEquals, assertMatch } from '#test/assert';
...
test('tRPC unknown procedure returns NOT_FOUND with aligned appCode/httpStatus', async () => {
...
test('removed config settings procedures return NOT_FOUND', async () => {
  const appModule = await import('../app.ts');
...
test('tRPC routes reject missing Authorization header', async () => {
  const appModule = await import('../app.ts');
...
test('tRPC query routes accept POST method override before auth', async () => {
  const appModule = await import('../app.ts');
...
test('tRPC routes ignore x-client-authorization fallback header', async () => {
  const appModule = await import('../app.ts');
  const response = await appModule.default.request('/trpc/data.profile', {
    method: 'POST',
    headers: {
