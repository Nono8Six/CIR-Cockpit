...
test('config integrity targeted correction contract is strict', () => {
  const payload = {
...
test('dataEntitiesPayloadSchema supports delete action for super-admin workflows', () => {
  const deletePayload = {
    action: 'delete',
...
test('dataEntitiesPayloadSchema supports entity list and search index actions', () => {
  const listPayload = {
    action: 'list',
    entity_type: 'Client',
    agency_id: '11111111-1111-4111-8111-111111111111',
...
test('dataEntitiesPayloadSchema rejects entity departments outside the live table constraint', () => {
  const clientPayload = {
    action: 'save',
    agency_id: '11111111-1111-4111-8111-111111111111',
    entity_type: 'Client',
...
test('dataEntitiesPayloadSchema supports individual client creation with primary contact', () => {
  const payload = {
    action: 'save',
    agency_id: '11111111-1111-4111-8111-111111111111',
    entity_type: 'Client',
...
test('dataEntitiesPayloadSchema supports explicit primary contact selection for company and prospect', () => {
...
test('dataEntitiesPayloadSchema supports supplier save action', () => {
...
test('dataEntityContactsPayloadSchema supports list_by_entity, save and delete actions', () => {
  const savePayload = {
    action: 'save',
...
test('dataInteractionsPayloadSchema supports save, add_timeline_event, agency lists, drafts and delete', () => {
  const savePayload = {
    action: 'save',
    agency_id: '11111111-1111-4111-8111-111111111111',
    interaction: {
...
test('directory.list refuses unknown and legacy fields', () => {
  const validPayload = {
...
test('directory contracts accept scoped payloads and reject legacy shortcut filters', () => {
...
test('directory contracts parse all_accessible_agencies only as explicit scope value', () => {
...
test('directory saved view state accepts migrated scope and rejects legacy agency fields', () => {
  const migratedState = {
    q: 'acme',
    type: 'all',
    scope: {
...
test('directory saved views list accepts referential segment views', () => {
...
test('tier V1 payload contracts accept specialized product types and reject generic mixing', () => {
  const agencyId = '11111111-1111-4111-8111-111111111111';
  const validSupplier = {
    tier_type: 'supplier',
    agency_id: agencyId,
...
test('tier V1 solicitation contract remains interaction-only', () => {
  const payload = {
...
test('tier V1 unified search and canonical tier directory contracts are strict', () => {
  const searchPayload = {
    query: 'acme',
    family: 'clients',
    client_filter: 'term',
...
test('tier V1 directory rows cover multi-source annuaire results', () => {
  const row = {
