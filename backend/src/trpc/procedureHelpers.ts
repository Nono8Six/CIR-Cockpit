...
type AuthedUserAndServiceHandler<TInput, TOutput> = (
  userDb: DbClient,
  serviceDb: DbClient,
  authContext: AuthContext,
  requestId: string,
...
export const withAuthedUserAndServiceHandler = <TInput, TOutput>(
  handler: AuthedUserAndServiceHandler<TInput, TOutput>
) => {
  return async (
    { ctx, input }: { ctx: AuthedProcedureContext; input: TInput }
  ): Promise<TOutput> => {
    try {
      ...
      return await handler(ctx.userDb, ctx.db, ctx.authContext, ctx.requestId, input);
    } catch (error) {
      return handleProcedureError(error);
    }
  };
