...
test('persistSelectedPrimaryContact demotes old primary and promotes selected active contact', async () => {
...
test('persistSelectedPrimaryContact accepts no primary contact for company and prospect records', async () => {
  const { db, updates } = createPrimaryContactDbMock();
...
test('persistSelectedPrimaryContact rejects missing or archived selected contact', async () => {
  const { db, updates } = createPrimaryContactDbMock([]);
...
test('persistEntityRow allows synced official field update during explicit resync', async () => {
  const mock = createEntityPersistDbMock();
...
test('persistEntityRow applies only selected fields during official resync', async () => {
  const mock = createEntityPersistDbMock();
...
test('persistEntityRow rejects official resync when base SIREN differs from persisted identity', async () => {
  const mock = createEntityPersistDbMock();
...
test('persistEntityRow allows manual official candidate selection when no identity is persisted', async () => {
...
test('persistEntityRow rejects manual official candidate selection when identity is already persisted', async () => {
  const mock = createEntityPersistDbMock();
