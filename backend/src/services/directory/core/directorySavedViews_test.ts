...
test('enforceDirectorySavedViewsLimit allows updates regardless of the existing count', () => {
  enforceDirectorySavedViewsLimit(DIRECTORY_SAVED_VIEWS_LIMIT, false);
});

test('enforceDirectorySavedViewsLimit rejects creates beyond the limit', () => {
...
test('requireDirectorySavedView returns the first row when present', () => {
  const row = requireDirectorySavedView([{ id: 'view-1' }], 'Vue sauvegardee introuvable.');
  assertEquals(row.id, 'view-1');
});

test('requireDirectorySavedView throws a not found app error when no row is returned', () => {
  const error = assertThrows(() =>
