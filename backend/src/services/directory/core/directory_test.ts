...
import { mapEnterpriseApiCompanyDetails } from '../company/directoryCompanyDetailsMapper.ts';
import { enterpriseApiSearchResponseSchema } from '../company/directoryCompanySchemas.ts';
...
test('normalizeBooleanFlag normalizes booleans, numbers and known strings', () => {
...
test('normalizeTextArray trims, dedupes and drops empty entries', () => {
  assertEquals(
...
test('buildCompanySearchUrl includes optional filters and pagination', () => {
...
test('enterprise API details accepts string director birth years', () => {
...
test('buildCompanyDuplicateReason prioritizes SIRET duplicates', () => {
...
test('buildCompanyDuplicateReason falls back to SIREN when SIRET is absent', () => {
  const input: Extract<DirectoryDuplicatesInput, { kind: 'company' }> = {
    kind: 'company',
    scope: { mode: 'active_agency' },
    includeArchived: true,
...
test('rankIndividualDuplicate gives the strongest rank to an email match', () => {
...
test('rankIndividualDuplicate falls back to phone matching when email differs', () => {
  const input: Extract<DirectoryDuplicatesInput, { kind: 'individual' }> = {
    kind: 'individual',
    scope: { mode: 'active_agency' },
    includeArchived: true,
...
test('rankIndividualDuplicate returns null when identity fields do not match', () => {
  const input: Extract<DirectoryDuplicatesInput, { kind: 'individual' }> = {
    kind: 'individual',
    scope: { mode: 'active_agency' },
    includeArchived: true,
