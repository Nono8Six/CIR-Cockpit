...
test("ensurePricingReferenceFileAccepted rejects invalid extension and size with app codes", () => {
  ...
  assertEquals(
    readErrorProperty(invalidExtension, "code"),
  ...
  assertEquals(
    readErrorProperty(tooLarge, "code"),
...
test("analyzePricingReferenceWorkbooks computes deterministic counters and French anomalies", async () => {
  ...
  assertEquals(
    pricingReferenceHealthReportSchema.safeParse(result.health_report).success,
    true,
  );
  ...
  assertEquals(
    result.health_report.segments_grids.classification_incomplete_rows,
    2,
  );
  assertEquals(
    result.health_report.segments_grids.cir_keys_not_validated_rows,
    3,
  );
  assertEquals(
    result.health_report.segments_grids.purchase_grid_missing_rows,
    1,
  );
  assertEquals(
    result.anomalies.some((anomaly) =>
      anomaly.message === "Champ grille achat structurel manquant."
    ),
    true,
  );
  assertEquals(
    result.anomalies.some((anomaly) =>
      anomaly.message.includes("Cle CIR non reconnue")
    ),
    true,
  );
...
test("inspectPricingReferenceWorkbook detects exact canonical columns automatically", () => {
  const classificationBytes = workbookBytes("Classification", [
    [...CLASSIFICATION_EXPECTED_COLUMNS],
    ["1", "2", "3", "Mega", "Fam", "Sous-famille"],
  ]);
  ...
  assertEquals(
    inspection.candidates.every((candidate) => candidate.status === "auto"),
    true,
  );
...
test("inspectPricingReferenceWorkbook reuses saved aliases when source columns changed", () => {
  const classificationBytes = workbookBytes("Classification", [
    [
      "MEGA",
      "FAM",
      "SFA",
      "Libelle mega produit",
      "Libelle famille produit",
      "Libelle sous famille produit",
    ],
    ["1", "2", "3", "Mega", "Fam", "Sous-famille"],
  ]);
  ...
  assertEquals(
    inspection.candidates.find((candidate) =>
      candidate.canonical_column === "MEGA_LIB"
    )?.status,
    "alias",
  );
...
test("listPricingReferenceCanonicalSourceRows fills canonical raw values from confirmed mapping", () => {
  const classificationBytes = workbookBytes("Classification", [
    [
      "Mega code",
      "Famille code",
    ...
    ["9", "20", "99", "Outillage", "Outillage a main", "Divers"],
  ]);

  const rows = listPricingReferenceCanonicalSourceRows({
    file_kind: "classification",
    original_filename: "Classification.xlsx",
    bytes: classificationBytes,
    sha256: "a".repeat(64),
...
test("analyzePricingReferenceWorkbooks accepts manual column mapping when classification headers changed", async () => {
  const classificationBytes = workbookBytes("Classification", [
    [
      "Mega code",
      "Famille code",
...
test("assertPricingReferenceCurrentMappingsConfirmed refuses newly uploaded files without confirmed mapping", () => {
...
test("analyzePricingReferenceWorkbooks preserves raw values and normalizes controlled CIR fields", async () => {
  const classificationBytes = workbookBytes("Classification", [
    [...CLASSIFICATION_EXPECTED_COLUMNS],
  ...
  assertEquals(
    result.purchase_grid_rows[0]?.date_debut_normalized,
    "2026-01-01",
  );
...
test("analyzePricingReferenceWorkbooks flags unknown CIR date formats without inventing dates", async () => {
  const classificationBytes = workbookBytes("Classification", [
    [...CLASSIFICATION_EXPECTED_COLUMNS],
    ["1", "2", "3", "Mega", "Fam", "Sous-famille"],
  ]);
