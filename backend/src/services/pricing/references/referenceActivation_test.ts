...
test("reference activation activates an analyzed snapshot without previous active version", async () => {
...
test("reference activation rejects an already active snapshot", async () => {
  const db = createFakeDb([
    [{ id: importId, status: "analyse_ok" }],
...
test("reference activation rolls back to an older analyzed version and keeps imports unchanged", async () => {
  const db = createFakeDb([
    [{ id: importId, status: "analyse_ok" }],
    [{ id: snapshotId, is_active: false }],
...
test("reference activation rejects imports that are not analyse_ok", async () => {
  const db = createFakeDb([
...
test("reference snapshot resolver prioritizes active snapshot then falls back to latest analyse_ok", async () => {
