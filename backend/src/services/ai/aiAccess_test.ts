...
test("assistant access maps database failures without leaking SQL details", async () => {
  const db = {
...
test("assistant access resolves user before active agency before global", () => {
...
test("assistant access ignores agencyIds when activeAgencyId is null", () => {
...
test("assistant access gives superadmin an unconditional bypass", () => {
  assertEquals(
    resolveAssistantAccessFromGrants(
      auth({
        role: "super_admin",
...
test("usage by member returns named strict per-feature consumption", async () => {
  const db = {
    execute: () =>
      Promise.resolve([{
        user_id: "00000000-0000-4000-8000-000000000001",
