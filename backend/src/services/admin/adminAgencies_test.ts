...
test('handleAdminAgenciesAction is exported as a function', () => {
  assert(typeof handleAdminAgenciesAction === 'function');
});

test('isAgencyNameConflictError detects postgres unique violations', () => {
  assertEquals(
...
test('handleAgencyNameConflict maps unique violation to 409 AGENCY_NAME_EXISTS', () => {
...
test('handleAgencyNameConflict maps unknown db error to 400 AGENCY_UPDATE_FAILED', () => {
  const error = assertThrows(() =>
