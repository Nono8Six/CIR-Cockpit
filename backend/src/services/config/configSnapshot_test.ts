...
import { departmentReferenceSchema } from '../../../../shared/schemas/system/config.schema.ts';
...
test('parseStoredJson returns parsed data for valid payloads', () => {
...
test('parseStoredJson throws a DB_READ_FAILED app error on invalid payloads', () => {
...
test('resolveConfigAgencyId returns the requested agency for an allowed member', () => {
  const agencyId = resolveConfigAgencyId(memberContext, ' agency-a ');
  assertEquals(agencyId, 'agency-a');
});

test('resolveConfigAgencyId uses the single membership when no agency id is provided', () => {
...
test('resolveConfigAgencyId returns null for a super admin without target agency', () => {
  assertEquals(resolveConfigAgencyId(superAdminContext, undefined), null);
});

test('resolveConfigAgencyId rejects missing agency ids for multi-agency non super admins', () => {
...
test('mapAgencyReferenceStatuses keeps valid categories', () => {
...
test('mapAgencyReferenceStatuses throws when the stored category is invalid', () => {
  const error = assertThrows(() =>
    mapAgencyReferenceStatuses([
      {
        id: 'status-1',
...
test('departmentReferenceSchema accepts Corsica department codes', () => {
