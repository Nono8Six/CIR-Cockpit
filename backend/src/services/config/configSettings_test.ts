...
test('normalizeLabelList trims whitespace', () => {
  assertEquals(normalizeLabelList(['  foo  ', 'bar']), ['foo', 'bar']);
});

test('normalizeLabelList drops empty and whitespace-only labels', () => {
  assertEquals(normalizeLabelList(['', '   ', 'a']), ['a']);
});

test('normalizeLabelList dedupes case-insensitively and keeps first occurrence casing', () => {
  assertEquals(normalizeLabelList(['Foo', 'foo', 'FOO', 'bar']), ['Foo', 'bar']);
});

test('normalizeLabelList preserves order', () => {
  assertEquals(normalizeLabelList(['c', 'a', 'b']), ['c', 'a', 'b']);
});

test('assertValidStatusCategories accepts known categories', () => {
  assertValidStatusCategories([
...
test('assertValidStatusCategories throws on unknown category', () => {
  const invalidRows = [
    { id: undefined, label: 'X', category: 'bogus' }
  ] as unknown as Parameters<typeof assertValidStatusCategories>[0];

  assertThrows(
    () => assertValidStatusCategories(invalidRows),
    Error,
    'Categorie de statut invalide'
  );
});

test('buildStatusUpsertRows sets sort_order by index and flags first as default', () => {
...
test('buildStatusUpsertRows marks done category as terminal', () => {
  const rows = buildStatusUpsertRows(
...
test('buildStatusUpsertRows preserves explicit id when provided', () => {
  const rows = buildStatusUpsertRows(
...
test('buildStatusUpsertRows resolves id by matching existing label case-insensitively', () => {
  const rows = buildStatusUpsertRows(
    [{ id: undefined, label: 'Nouveau', category: 'todo' }],
    'agency-1',
...
test('buildStatusUpsertRows leaves id undefined for new labels with no existing match', () => {
  const rows = buildStatusUpsertRows(
    [{ id: undefined, label: 'Nouveau', category: 'todo' }],
    'agency-1',
...
test('buildStatusUpsertRows trims label whitespace', () => {
  const rows = buildStatusUpsertRows(
    [{ id: undefined, label: '  Nouveau  ', category: 'todo' }],
    'agency-1',
    []
...
test('buildStatusUpsertRows sets agency_id on every row', () => {
  const rows = buildStatusUpsertRows(
    [
      { id: undefined, label: 'A', category: 'todo' },
      { id: undefined, label: 'B', category: 'in_progress' }
...
test('ensureOneAffectedRow rejects stale concurrent updates', () => {
  assertThrows(
...
test('ensureOneAffectedRow accepts exactly one updated row', () => {
