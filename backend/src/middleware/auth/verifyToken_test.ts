...
test('verifyToken.getBearerToken extracts bearer token case-insensitively', () => {
...
test('verifyToken.getAccessTokenFromHeaders returns bearer token', () => {
  assertEquals(getAccessTokenFromHeaders('Bearer gatewayToken'), 'gatewayToken');
  assertEquals(getAccessTokenFromHeaders(null), '');
});

test('verifyToken.createJwtAuthGateway verifies a valid token', async () => {
...
test('verifyToken.createJwtAuthGateway rejects disallowed algorithm', async () => {
  const issuer = 'https://example.supabase.co/auth/v1';
  const audience = 'authenticated';
