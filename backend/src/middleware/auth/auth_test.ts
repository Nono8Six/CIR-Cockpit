...
test('getBearerToken returns empty string when header is missing', () => {
  assertEquals(getBearerToken(null), '');
});

test('getBearerToken extracts bearer token case-insensitively', () => {
  assertEquals(getBearerToken('Bearer abc123'), 'abc123');
  assertEquals(getBearerToken('bearer xyz'), 'xyz');
});

test('getBearerToken returns empty when scheme is invalid', () => {
  assertEquals(getBearerToken('Token abc123'), '');
});

test('getAccessTokenFromHeaders prioritizes Authorization', () => {
  assertEquals(
...
test('getAccessTokenFromHeaders returns empty when Authorization is missing', () => {
  assertEquals(getAccessTokenFromHeaders(null), '');
});

test('isProfileAccessRevoked returns true for archived profiles', () => {
  assertEquals(
...
test('isProfileAccessRevoked returns true for system profiles', () => {
  assertEquals(
...
test('createJwtAuthGateway verifies a valid token and extracts sub', async () => {
...
test('createJwtAuthGateway rejects invalid issuer', async () => {
  const issuer = 'https://example.supabase.co/auth/v1';
  const audience = 'authenticated';
  const { publicKey, privateKey } = await generateKeyPair('ES256');
  const publicJwk = await exportJWK(publicKey) as JWK;
...
test('createJwtAuthGateway rejects disallowed algorithm', async () => {
  const issuer = 'https://example.supabase.co/auth/v1';
  const audience = 'authenticated';
