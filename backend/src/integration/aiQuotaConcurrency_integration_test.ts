...
const enabled = process.env.RUN_AI_DB_EVALS === "1";
const databaseUrl = process.env.DATABASE_URL?.trim() ?? "";

integrationTest({
  name: "AI wildcard quota is atomic across two features and idempotent under concurrency",
  ...
  fn: async () => {
    ...
    try {
      await sql`
        ...
        ) values ('user', ${userId}::uuid, null, true, 5, 5)
      `;
